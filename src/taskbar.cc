...
namespace gzz {
	...
	void Taskbar::refreshbar()
	{
		...
		for(auto win : lst){
			if(!win->is_skip_tasklist() && win->is_on_workspace(*workspace) && win->is_in_viewport(*workspace) && m_mappings.find(win) == m_mappings.end()){
				...
				task->show_all();
				task->set_fontdescription(m_fontdescription);
				task->set_colour(m_fontColour);
			    ...
			    /*std::cerr << __FILE__ << '[' << __LINE__ << "] " << __PRETTY_FUNCTION__ 
				          << " getting class_group " << std::endl;*/
				Wnck::ClassGroup *class_group = win->get_class_group();
			    /*std::cerr << __FILE__ << '[' << __LINE__ << "] " << __PRETTY_FUNCTION__ 
				          << " got class_group == " << class_group << std::endl;*/
				if(!class_group) continue;
			    /*std::cerr << __FILE__ << '[' << __LINE__ << "] " << __PRETTY_FUNCTION__ 
				          << "\t class_group good: class_group " << class_group << std::endl;*/
				std::vector<Wnck::Window*> vec = class_group->get_windows();
				std::vector<Wnck::Window*> vec2 = get_windows_on_viewport(vec, workspace);
				if(m_class_group_mappings.find(class_group->gobj()) == m_class_group_mappings.end() && m_max_group_size < vec2.size()){
					m_max_class_group = class_group;
					m_max_group_size  = vec2.size();
				    /*std::cerr << __FILE__ << '[' << __LINE__ << "] " << __PRETTY_FUNCTION__ 
					          << " m_max_group_size == " << m_max_group_size << std::endl;*/
				}
				/*std::cerr << __FILE__ << '[' << __LINE__ << "] " << __PRETTY_FUNCTION__ 
					      << " done with class_group" << std::endl;*/
				if(m_mappings.size() > 0 && m_mappings.find(win) != m_mappings.end()){
					/*std::cerr << __FILE__ << '[' << __LINE__ << "] " << __PRETTY_FUNCTION__ 
					          << " inside should I be: win->get_name() == " << win->get_name() << std::endl;*/
					//regex
					//if(win->get_name()
					Task* task = m_mappings[win];
					/*std::cerr << __FILE__ << '[' << __LINE__ << "] " << __PRETTY_FUNCTION__ 
					          << " inside should I be: task->get_constructed() == " 
						      << std::boolalpha << task->get_constructed() << std::endl;
					std::cerr << __FILE__ << '[' << __LINE__ << "] " << __PRETTY_FUNCTION__ 
					          << " inside should I be: task->get_destructed() == " 
						      << std::boolalpha << task->get_destructed() << std::endl;*/
					task->refresh_state();
				}
	...
	{
		/*std::cerr << __FILE__ << '[' << __LINE__ << "] " << __PRETTY_FUNCTION__ 
				  << " entering: vec.size() == " << vec.size() << std::endl;*/
	...
	void Taskbar::erase_task_from_class_group_mappings_by_window(Wnck::Window* win)
	{
		WnckClassGroup* class_group = nullptr;  // probably already dead //
		for(auto p : m_class_group_mappings){
			if(p.second->contains(win)) class_group = p.first;
		}
		if(class_group){
			m_class_group_mappings.erase(class_group);
		}
	...
	Pango::FontDescription Taskbar::get_fontdescription() const
	{
		return m_fontdescription;
	}

	void Taskbar::set_fontdescription(Pango::FontDescription fd)
	{
		m_fontdescription = fd;
		for(auto p : m_mappings){
			p.second->set_fontdescription(m_fontdescription);
		}
		for(auto p : m_class_group_mappings){
			p.second->set_fontdescription(m_fontdescription);
		}
	...
	Gdk::RGBA Taskbar::get_colour() const
	{
		return m_fontColour;
	}

	void Taskbar::set_colour(Gdk::RGBA rgba)
	{
		m_fontColour = rgba;
		for(auto p : m_mappings){
			p.second->set_colour(m_fontColour);
		}
		for(auto p : m_class_group_mappings){
			p.second->set_colour(m_fontColour);
		}
